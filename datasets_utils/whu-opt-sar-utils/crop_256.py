...
class image_to_patch:
    ...
    def to_patch(self):
        ...
        for file_name in sar_files[self.left : self.right]:
            ...
            #             "====> sar save",
            #             os.path.join(
            #                 self.crop_sar_image_path, prefix + "_" + str(n_sar) + ".tif"
            #             ),
            #         )
            #         sub_img_label.save(
            #             os.path.join(
            ...
            for x in range(0, h - self.stride, self.stride):
                for y in range(0, w - self.stride, self.stride):
                    ...
                    print(
                        "====> optical save",
                        os.path.join(
                            self.crop_optical_image_path,
                            prefix + "_" + str(n_optical) + ".tif",
                        ),
                    )
                    sub_img_label.save(
                        os.path.join(
...
if __name__ == "__main__":
    ...
    sar_path = r"D:\Code\d2-net\datasets\whu-opt-sar\sar"
    crop_sar_image_path = r"D:\Code\d2-net\datasets\whu-opt-sar\crop\sar"

    optical_path = r"D:\Code\d2-net\datasets\whu-opt-sar\optical"
    crop_optical_image_path = r"D:\Code\d2-net\datasets\whu-opt-sar\crop\optical"
