...
def eval_summary_homography(dists_sa, dists_si, dists_sv, thres):
    ...
    # Generate summary
    summary = f'Homography estimate Correct: a={correct_sa}\ni={correct_si}\nv={correct_sv}\n'
    summary += f'Homography estimate AUC: a={auc_sa}\ni={auc_si}\nv={auc_sv}\n'
    return summary
...
def eval_summary_matching(results, thres=[1, 3, 5, 10], save_npy=None):
    ...
    n_i = 52
    n_v = 56
    i_err, v_err, stats,NCM_,CMR_,RMSE_ = results
    ...
    summary += '#Matches: a={:.0f}, i={:.0f}, v={:.0f},NCM={:.2f},CMR={:.2f},RMSE={:.2f}\n'.format(
                    np.mean(n_matches),
...
def eval_hpatches(
    ...
    for seq_idx, seq_dir in tqdm(enumerate(seq_dirs[::-1]), total=len(seq_dirs), smoothing=.5):
        ...
        for im_idx in range(2, 7):
            ...
            try:
                ...
                # matches_H, p1s_H, p2s_H = match_res[0:3]
                if useDg:
            ...
            if 'matching' in task:
                ...
                filter_idx= []
                # 先用阈值3剔除欧式距离
                filter_idx = np.where(dist<3)
                NCM.append(len(filter_idx[0]))
                CMR.append(len(filter_idx[0])/len(p1s))
                filter_dist = dist[filter_idx[0]]
                # bool_list = dist<thres
                if filter_dist.size == 0:
                    RMSE.append(0)
                else:
                    RMSE.append(np.sqrt(np.mean(filter_dist**2)))
    ...
    if 'matching' in task:
        results = i_err, v_err, [np.array(seq_type), np.array(n_feats), np.array(n_matches)],np.array(NCM),np.array(CMR),np.array(RMSE)
